...
r.get('/:scope?/:name/-/:scope2?/:filename/:sha', middleware.auth.read, aw(async function (req, res) {
  ...
  if (sha.startsWith(filename)) {
    ...
    const urlPrefix = scope ? `/${scope}` : ''
    const url = `${urlPrefix}/${name}/-/${scope2}/${sha}`

    res.redirect(url)
    return
  }
  let key = pathJoinSafe('tarballs', scope ? `${scope}/${name}` : name, filename, sha)
  ...
  if (!tarball) {
    ...
    await config.storage.put(key, tarball.response, {
