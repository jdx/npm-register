...
function updateEtag(name, etag) {
  co(function* () {
    if (redis.status !== 'ready') { return rediserr(); }
    yield redis.setex(`${cacheKey(name)}/etag`, config.cache.npmPackages, etag);
  });
}
...
function updateCache (pkg) {
  co(function* () {
    if (redis.status !== 'ready') { return rediserr(); }
    yield redis.setex(cacheKey(pkg.name), config.cache.npmPackages, JSON.stringify(pkg));
  });
}

function* get(name, etag) {
  try {
    if (etag && (yield isEtagFresh(name, etag))) { return 304; }
