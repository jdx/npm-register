...
function* savePkg (pkg) {
  ...
  yield s3.upload({Key: `packages/${pkg.name}`, Body: JSON.stringify(pkg), ContentType: 'application/json'});
}
...
function* upload (pkg) {
  ...
  for (let filename of Object.keys(attachments)) {
    ...
    yield s3.upload({Body: data, Key: `tarballs/${pkg.name}/${filename}`, ContentType: attachment.content_type, ContentLength: attachment.length});
  }
  yield savePkg(pkg);
}
