...
module.exports = function (metric) {
  ...
  function * savePkg (pkg) {
    metric.profile({'storage.save_pkg': pkg.name})
    let data = new Buffer(JSON.stringify(pkg))
    yield storage.putBufferAsync(data, `/packages/${pkg.name}`, {
  ...
  function refreshPkg (npmPkg) {
    co(function * () {
      let storagePkg = yield storage.download(`/packages/${npmPkg.name}`)
      if (!storagePkg) {
        yield savePkg(npmPkg)
        return
      }
      storagePkg = JSON.parse(storagePkg)
  ...
  function * get (name, etag) {
    ...
    if (pkg === 404) {
      ...
      if (!pkg) return 404
      metric.event({'storage.serve': name})
  ...
  function * upload (pkg) {
    ...
    for (let filename of Object.keys(attachments)) {
      ...
      yield storage.putBufferAsync(data, `/tarballs/${pkg.name}/${filename}/${sha}${ext}`, {
